    ...
    public class GSplineEditingGUIDrawer
    {
        ...
        private void DrawMesh()
        {
            ...
            if (GEditorSettings.Instance.splineTools.showMesh)
            {
                ...
                int trisCount = splineCreator.Editor_Vertices.Count / 3;
                for (int i = 0; i < trisCount; ++i)
                {
                    Vector4 p0 = splineCreator.Editor_Vertices[i * 3 + 0];
                    Vector4 p1 = splineCreator.Editor_Vertices[i * 3 + 1];
                    Vector4 p2 = splineCreator.Editor_Vertices[i * 3 + 2];
