...
#include "UnityCG.cginc"
	...
	fixed4 fragPaint(v2f i) : SV_Target
	{
		...
		float addictiveValue = maskGray * _Color.r * _Opacity * terrainMask;
		...
		fixed4 fragErase(v2f i) : SV_Target
	{
		...
		float4 currentColor = tex2D(_MainTex, i.localPos);
		float currentValue = currentColor.r;
		float subtractiveValue = maskGray * _Color.r * _Opacity * terrainMask;
		...
		fixed4 fragSmooth(v2f i) : SV_Target
	{
		...
		float4 desColor = lerp(currentColor, avgColor, maskValue * _Opacity * terrainMask);
